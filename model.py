...
class convLSTMnet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(in_features=241*100, out_features=1)
    ...
    def forward(self, x):
        ...
        x = x.float()
        x = x.permute(0, 2, 1)
        # print(x.size())  #torch.Size([90, 21, 256])
        x = F.relu(self.conv1(x))
        # print(x.size()) #torch.Size([90, 64, 241])
        #torch.Size([batchsize, 64, 985])
        x = x.permute(0, 2, 1)
        #torch.Size([batchsize, 985, 64])
        x = F.max_pool1d(x, 5, stride=5)
        # print(x.size())  #torch.Size([90, 241, 12])
        ...
        x = output
        #torch.Size([72, 100])
        # print(x.size())  #torch.Size([90, 241, 100])
        x = x.contiguous().view(-1, 241*100)  # 应与nn.linear中一致
        # print(x.size())
        #torch.Size([72, 100])
        x = self.fc1(x)
