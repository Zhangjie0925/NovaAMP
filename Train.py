...
# 	x_valid = torch.tensor(format(args.inputs)[0], dtype=torch.long)
#     y_valid_label = torch.tensor(format(args.inputs)[1], dtype=torch.float)
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            log('[epoch: %d / batch: %d] train_loss: %.3f' %
                (epoch+1, i + 1, loss.item()), 'train.log')
        ...
        #     precision_value, auroc_value, f1_value), './model.'+'.everyepoch.valid.txt')
        # if f1_value > best_f1:
        #     best_f1 = f1_value
    ...
    #     valid_loss, accuracy_value, recall_value, precision_value, auroc_value, f1_value), 'valid.log')
